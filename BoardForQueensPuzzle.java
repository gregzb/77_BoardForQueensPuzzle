...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
        ...
        int lastAdded = filesWithQueens[lastRankFilled];
        for (int idx = 0; idx < lastRankFilled; idx++) {
            if (lastAdded == filesWithQueens[idx]) return true;
            if (Math.abs(lastRankFilled-idx)==Math.abs(lastAdded-filesWithQueens[idx])) return true;
        }
        return false;
    }
    ...
    public boolean accept() {
        System.out.println(lastRankFilled + ", " + (ranks() - 1) + ", " + lastIsNg());
        return lastRankFilled == ranks() - 1 && !lastIsNg();
    }
